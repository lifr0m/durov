...
fn write_layer(writer: &mut Writer, layer: u16) {
    writer.indent_write("pub const LAYER: u16 = ");
...
fn group_types(items: Vec<Type>) -> Namespace<Namespace<Type>> {
    ...
    Namespace {
        name: Some(String::from(TYPES_NAMESPACE)),
        items,
    }
}
...
fn group_combinators(items: Vec<&Combinator>, typ: CombinatorType) -> Namespace<Namespace<&Combinator>> {
    ...
    Namespace {
        name: Some(String::from(match typ {
            CombinatorType::Constructor => CONSTRUCTORS_NAMESPACE,
            CombinatorType::Function => FUNCTIONS_NAMESPACE,
        })),
        items,
    }
}
...
fn map_namespaces(schema: &Schema, types: &[Type]) -> HashMap<Name, String> {
    ...
    for combinator in &schema.combinators {
        map.insert(combinator.name.clone(), String::from(match combinator.typ {
            CombinatorType::Constructor => CONSTRUCTORS_NAMESPACE,
            CombinatorType::Function => FUNCTIONS_NAMESPACE,
        }));
    }
    ...
    for item in types {
        map.insert(item.name.clone(), String::from(TYPES_NAMESPACE));
    }
